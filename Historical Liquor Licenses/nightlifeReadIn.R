...
#standardize columns and variable names
#standardize columns and variable names
abc0812 <- abc0812 %>%  subset(grepl(" CN | CR | CT", Description)) %>%
  rename(license = License, orgName = Applicant.Name, name08 = Trade.Name, type = Description) %>%
  mutate(license=gsub( "\\.|/|\\-|\"|\\s", "",license),
         address08=(paste(Street.Number, Street.Name, ST.Type, Quad))) %>%
  select(license, name08, address08)


abc0912 <- abc0912 %>%  subset(grepl(" CN | CR | CT", Description)) %>%
  rename(license = License, orgName = Applicant.Name, name09 = Trade.Name, type = Description) %>%
  mutate(license=gsub( "\\.|/|\\-|\"|\\s", "",license),
         address09=(paste(Street.., Street.Name, Street.Type, Quad))) %>%
  select(license, name09, address09)


abc1011 <- abc1011 %>%  subset(grepl(" CN | CR | CT", Description)) %>%
  rename(license = License, orgName = Applicant, name10 = Trade.Name, type = Description) %>%
  mutate(license=gsub( "\\.|/|\\-|\"|\\s", "",license),
         ...
         select(license, name10, address10) 


abc1110 <- abc1110 %>%  subset(grepl("Nightclub|Restaurant|Tavern", TYPE)) %>%
  rename(license = LICENSE.NUMBER, orgName = APPLICANT.NAME, name11 = LICENSEE.TRADE.NAME, type = TYPE) %>%
  mutate(license=gsub( "\\.|/|\\-|\"|\\s", "",license),
         address11= sub("\\\n.*", "", ADDRESS)) %>%
  select(license, name11, address11)
 

abc1212 <- abc1212 %>%  subset(grepl("Nightclub|Restaurant|Tavern", Establishment.Type)) %>%
  rename(license = License.Number, name12 = Trade.Name, orgName = Organization.Name, type = Establishment.Type) %>%
  mutate(license=gsub( "\\.|/|\\-|\"|\\s", "",license),
         address12= sub("\\\n.*", "", Address)) %>%
  select(license, name12, address12)
  

abc1308 <- abc1308 %>%  subset(grepl("Nightclub|Restaurant|Tavern", DESCRIPTION)) %>%
  rename(license = LICENSE, orgName = APPLICANT, name13 = TRADE.NAME, type = DESCRIPTION, status = STATUS) %>%
  mutate(license=gsub( "\\.|/|\\‐|\"|\\s|-", "",license),
         address13= sub("\\\n.*", "", ADDRESS)) %>%
  select(license, name13, address13)


abc1408 <- abc1408 %>%  subset(grepl("Nightclub|Restaurant|Tavern", Description)) %>%
  rename(license = License.., status = Status, orgName = Applicant, name14 = Trade.Name, type = Description) %>%
  mutate(license=gsub( "\\.|/|\\-|\"|\\s", "",license),
         address14=paste(Street.., Street.Name, Type, Quad)) %>%
  select(license, name14, address14)

  
abc1507 <- abc1507 %>%  subset(grepl("Nightclub|Restaurant|Tavern", Description)) %>%
  rename(license = License, status = Status, orgName = Applicant, name15 = TradeName, type=Description) %>%
  mutate(license=gsub( "\\.|/|\\-|\"|\\s", "",license),
         address15=paste(StreetNumber, StreetName, StreetType, Quadrant)) %>%
  select(license, name15, address15)
  

abc1608 <- abc1608 %>%  subset(grepl("Nightclub|Restaurant|Tavern", Type)) %>%
  rename(status = Status, orgName = Entity.Name, name16 = Trade.Name, type = Type) %>%
  mutate(license=gsub( "\\.|/|\\-|\"|\\s", "",License..),
         address16= sub("\\,.*", "", Address)) %>%
  select(license, name16, address16)
